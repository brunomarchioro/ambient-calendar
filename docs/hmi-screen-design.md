...
| Fonte       | **Montserrat** built-in LVGL — `lv_font_montserrat_20` (corpo) + `lv_font_montserrat_28` (relógio) |
...
| `COLOR_SYNC`    | `#00FF41` | texto `SYNC` (fresco)                                          |
...
| Card secundário (Agora+Alert)| 160 | 96     | #11–#16      |
| Lista / faixa livre          | 160 | 160    | #11–#20      |

Margens horizontais: **8 px** (`HMI_PAD_X`). Largura útil: **156 px**.
...
| `sync_lbl`             | label | 20    | `SYNC` ou `12m` (direita)                 |
...
| Condição                                      | Exibição        |
| --------------------------------------------- | --------------- |
| Relógio válido e cache &lt; **5 min**         | `SYNC` verde    |
| Cache entre **5** e **60 min**                | `Nm` cinza      |
...
3. Fontes: Montserrat built-in (`sdkconfig.defaults`).
