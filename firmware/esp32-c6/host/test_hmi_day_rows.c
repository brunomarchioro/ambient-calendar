...
static void assert_row(const hmi_row_t *row, hmi_row_kind_t kind, size_t event_index)
{
...
static void test_today_only(void)
{
	...
	assert(n == 1);
	assert_row(&rows[0], HMI_ROW_EVENT, 0);
}
...
static void test_first_item_tomorrow(void)
{
	...
	const size_t n = hmi_day_rows_plan(TODAY, keys, 1, 4, rows);
	assert(n == 2);
	assert_row(&rows[0], HMI_ROW_DAY_HEADER, 0);
	assert_row(&rows[1], HMI_ROW_EVENT, 0);
}
...
static void test_multi_day_full_budget(void)
{
	...
	assert(n == 6);
	assert_row(&rows[0], HMI_ROW_EVENT, 0);
	assert_row(&rows[1], HMI_ROW_DAY_HEADER, 1);
	assert_row(&rows[2], HMI_ROW_EVENT, 1);
	assert_row(&rows[3], HMI_ROW_EVENT, 2);
	assert_row(&rows[4], HMI_ROW_DAY_HEADER, 3);
	assert_row(&rows[5], HMI_ROW_EVENT, 3);
}
